...
与可扩展性的影响。
### 1.任务调度差异 
- 验证 MapReduce 的“启动-销毁”模式与 Giraph 的“常驻进程”模式的差异。
    - Metrics :  任务启动延时 、 每轮迭代平均耗时 
- 节点度分布不均时，验证Giraph 的顶点分区与 MR 的任务划分策略的差异。
    - Metrics : 不同节点资源占用率(内存、CPU、GC停顿时间)  评估负载均衡
### 2.迭代开销差异：MapReduce 磁盘 I/O vs  Giraph内存消息传递
- Variables : 不同的磁盘I/O速度、不同的网络环境(带宽、延迟、丢包)
- Metrics : HDFS读写字节数/OPS 本地磁盘写入     Giraph消息总数、消息字节数
